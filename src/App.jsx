...
const App = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (isEditing) {
      setEmploy(employees.map(emp => emp.id === employee.id ? employee : emp));
      setIsEditing(false);
    } else {
      setEmploy([...employees, { ...employee, id: Date.now().toString() }]);
    }
  ...
  const deleteEmployee = (id) => {
    setEmploy(employees.filter(emp => emp.id !== id));
  };
          ...
          <Card.Body>
            <Form onSubmit={handleSubmit}>
              ...
              <Row className='justify-content-center'>
                <Button variant="success" type="submit" className="mt-3 btn-width">
